...
namespace PathCreation {
 ...
 IsClosed = isClosed;
        ...
        public void AddSegment(Vector3 anchorPos, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
                {
                    ...
                    secondControlForOldLastAnchorOffset =
                        secondControlForOldLastAnchorOffset.normalized * dstPrevToNewAnchor * 0.5f;
                }

                Vector3 secondControlForOldLastAnchor = points[lastAnchorIndex] + secondControlForOldLastAnchorOffset;
                Vector3 controlForNewAnchor = (anchorPos + secondControlForOldLastAnchor) * 0.5f;
